...
(defun util-noise-aa (production)
  (let ((aa (get-module Affective-Associations))
   ...
   (if (<= arous-util-noise 0.5)
    (progn (sgp-fct (list :ut (* (AA-util-thresh-scalar aa) (- 1 arous-util-noise))));(- (AA-max-util-thresh aa) (* (- 1 (/ arous-util-noise 0.5)) (AA-max-util-thresh aa) ))))
	...
	(when (and (>= (mod (mp-time) 10) 0) (<= (mod (mp-time) 10) 1))
	  (with-open-file
		...
		(format msgStream "~10$~T" arous-util-noise)))
   ...
   (* (compute-homeostatic-arousal-factor) (compute-cort) 0.5 
	...
	(if (and (numberp pred-error-factor) (> pred-error-factor 0))
	 (/ pred-error-factor 3) 0))))
	  ...
	  ;; an arbitrary max amount of days awake of 4 before hitting a minimal value (0.001)
	  ;; The value output is in the range from 0 to 1

...
(defun dm-noise-aa (chunk)
 ...
 (let* ((aa (get-module Affective-Associations))
  ...
  (if (<= arous-dm-noise 0.5)
   (act-r-noise (/ (+ (* arous-dm-noise (AA-nom-dm-noise aa)) (* (- 0.5 arous-dm-noise) (AA-max-dm-noise aa))) 0.5))
   (act-r-noise (/ (+ (* (- 1 arous-dm-noise) (AA-nom-dm-noise aa)) (* (- arous-dm-noise 0.5) (AA-max-dm-noise aa))) 0.5)))))
...
(defun util-noise-aa (production)
  (let ((aa (get-module Affective-Associations))
   ...
   (when (<= arous-util-noise 0) (incf arous-util-noise 0.000001))
   (if (<= arous-util-noise 0.5)
    (progn (sgp-fct (list :ut (* (AA-util-thresh-scalar aa) (/ (- 1 arous-util-noise) 0.5))));(- (AA-max-util-thresh aa) (* (- 1 (/ arous-util-noise 0.5)) (AA-max-util-thresh aa) ))))
           (sgp-fct (list :egs (* (AA-util-noise-scalar aa) (- (/ (- 1 arous-util-noise) 0.5) 1))))) ;(AA-nom-util-noise aa))))));(/ (+ (* arous-util-noise (AA-nom-util-noise aa)) (* (- 0.5 arous-util-noise) (AA-max-util-noise aa))) 0.5))))
	(sgp-fct (list :egs (* (AA-util-noise-scalar aa) (- (/ arous-util-noise 0.5) 1)))));(+ (* (AA-util-noise-scalar aa) (/ arous-util-noise 0.5) (AA-nom-util-noise aa)) (* (AA-util-noise-scalar aa) (/ (- 1 arous-util-noise) 0.5) (AA-nom-util-noise aa))))));(/ (+ (* (- 1 arous-util-noise) (AA-nom-util-noise aa)) (* (- arous-util-noise 0.5) (AA-max-util-noise aa))) 0.5))))
  ...
  (let ((value 0.0)
    ...
    (when references
      (loop 
        ...
        do (progn
        (incf value (expt-coerced (max .05 (ms->seconds (- (mp-time-ms) reference))) (* minus-decay (- 1 arousal))))
...
(defun compute-arousal-factor (&optional pred-error-factor)
   (* (compute-homeostatic-arousal-factor) (compute-cort) 0.5 
    (+ 
    (if (and (get-module physio) (phys-module-enabled (get-module physio))) (compute-phys-arousal) 0)
    (if (and (get-module FEAR) (FEAR-enabled (get-module FEAR)))
	 (/ (FEAR-arousal (get-module FEAR)) (* 3 (log (exp (FEAR-max-FEAR-val (get-module FEAR)))))) 0)
	(if (and (numberp pred-error-factor) (> pred-error-factor 0))
	 (/ pred-error-factor 3) 0))))

	  ;;(2401-timeSinceLastAsleep^4)/2401 x from 0 to 7 where x=system.x/1000
(defun compute-homeostatic-arousal-factor ()
  (let* ((LA (if (cadar  (car (get-phys-vals nil (list '("Status.LastAsleep")))))
		...
		(if (and LA currT (> timeSinceAsleep 0))
		 (/ (- 2.65 (expt (/ timeSinceAsleep 1000) 0.5)) 2.65)
		 1)
  ))
...
(defun check-vis-loc-buffer-fear ()
 (let* ((f (get-module FEAR))
   ...
   (if exists
	(let ((chunk-FEAR-val (car exists)))
	 (when (AA-fear-dm-affect-hook a-a) (setf chunk-FEAR-val (funcall (AA-fear-vis-affect-hook a-a) chunk (AA-vis-FEAR-hash a-a))))
	 (schedule-event-relative delay 'update-vis-affect-val :params (list chunk) :module 'Affective-Associations :maintenance t)
   ...
   (if exists-s
	...
	(when (and (not exists) exists-s chunk) (schedule-event-relative delay 'update-dm-affect-val :params (list chunk) :module 'Affective-Associations :maintenance t))))
	 
	 

(defun check-vis-buffer-fear ()
(let* ((f (get-module FEAR))
		(s (get-module SEEKING))
	    (a-a (get-module Affective-Associations))
...
(defun check-aur-loc-buffer-fear ()
 (let* ((f (get-module FEAR))
	    (a-a (get-module Affective-Associations))
   ...
   (if exists
	(let ((chunk-FEAR-val (car exists)))
	 ...
	 (schedule-event-relative delay 'update-aur-affect-val :params (list chunk) :module 'Affective-Associations :maintenance t)
	 (setf (FEAR-val-association f) chunk-FEAR-val))
	(when chunk
     (if (AA-fear-aur-affect-hook a-a)
...
(defun check-retr-buffer-fear ()
  (let* ((f (get-module FEAR))
   ...
   (if exists
	(let ((chunk-FEAR-val (car exists)))
	 ...
	 (schedule-event-relative delay 'update-dm-affect-val :params (list chunk) :module 'Affective-Associations :maintenance t)
	 (setf (FEAR-val-association f) chunk-FEAR-val))
	(when chunk
     (if (AA-fear-aur-affect-hook a-a)
      (let ((chunk-FEAR-val nil)) 
 ...
 ;(schedule-event-after-module 'procedural 'check-retr-buffer-fear :module 'Affective-Associations))
...
(defun update-vis-affect-val (chunk)
 (let ((a-a (get-module Affective-Associations))
  ...
  (when (and val (> val (AA-FEAR-affect-thresh a-a)))
   (let* ((f (get-module FEAR))
		...
		(v-t (gethash chunk v-hash))
		(vis-mult (AA-vis-mult a-a))
		(k (AA-dm-k a-a))
        (g (/ 1 (+ 1 (* k (- (mp-time) (cadr v-t))))))
		(new-val (* vis-mult (+ (cadr v-t) (* alpha (+ r (- (* g fear-val) (car v-t))))))))
  (setf (gethash (hash-chunk-contents chunk) v-hash) (list new-val (mp-time))))))
 ;;SEEKING
  (let ((a-a (get-module Affective-Associations))
  ...
  (when (and val (> val (AA-seeking-affect-thresh a-a)))
   (let* ((s (get-module SEEKING))
		...
		(k (AA-dm-k a-a))
        (g (/ 1 (+ 1 (* k (- (mp-time) (cadr v-t))))))
		(new-val (* vis-mult (+ (cadr v-t) (* alpha (+ r (- (* g seeking-val) (car v-t))))))))
  (setf (gethash (hash-chunk-contents chunk) v-hash) (list new-val (mp-time)))))))

;;;affect-val = curr-val + alpha * (reinforcer + fear-state/(1 + k*t) - curr-val)
(defun update-aur-affect-val (chunk)
 (let ((a-a (get-module Affective-Associations))
	  (val (if chunk (chunk-fValue chunk) nil)))
  (when (and val (> val (AA-FEAR-affect-thresh a-a)))
   (let* ((f (get-module FEAR))
		...
		(chunk (hash-chunk-contents chunk))
		(v-t (gethash chunk a-hash))
		(aur-mult (AA-aur-mult a-a))
		(k (AA-aur-k a-a))
        (g (/ 1 (+ 1 (* k (- (mp-time) (cadr v-t))))))
		(new-val (* aur-mult (+ (cadr v-t) (* alpha (+ r (- (* g fear-val) (car v-t))))))))
  ...
  (when (and val (> val (AA-seeking-affect-thresh a-a)))
   (let* ((s (get-module SEEKING))
		...
		(k (AA-dm-k a-a))
        (g (/ 1 (+ 1 (* k (- (mp-time) (cadr v-t))))))
		(new-val (* aur-mult (+ (cadr v-t) (* alpha (+ r (- (* g seeking-val) (car v-t))))))))
  (setf (gethash (hash-chunk-contents chunk) a-hash) (list new-val (mp-time))))))
)

;;;affect-val = curr-val + alpha * (reinforcer + fear-state/(1 + k*t) - curr-val)
(defun update-dm-affect-val (chunk)
 ...
 (let ((a-a (get-module Affective-Associations))
  ...
  (when (and val (> val (AA-fear-affect-thresh a-a)))
   (let* ((f (get-module FEAR))
		...
		(k (AA-dm-k a-a))
        (g (/ 1 (+ 1 (* k (- (mp-time) (cadr v-t))))))
		(new-val (* dm-mult (+ (cadr v-t) (* alpha (+ r (- (* g fear-val) (car v-t))))))))
...
;;SEEKING
  ...
  (when (and val (> val (AA-seeking-affect-thresh a-a)))
   (let* ((s (get-module SEEKING))
		...
		(k (AA-dm-k a-a))
        (g (/ 1 (+ 1 (* k (- (mp-time) (cadr v-t))))))
		(new-val (* dm-mult (+ (cadr v-t) (* alpha (+ r (- (* g seeking-val) (car v-t))))))))
  (setf (gethash (hash-chunk-contents chunk) d-hash) (list new-val (mp-time))))))
)
...
(defun FEAR-chunk-activation-offset (chunk)
 (when chunk
  (let* ((f (get-module 'FEAR))
   ...
   (if (> f-c-v 1)
    (progn (print "Chunk offset test") (print (log f-c-v)) (log f-c-v))
    0))))
...
(defun schedule-aa-events (AA)
 (when (AA-enabled AA)
	 ...
	 (when (AA-chunk-affect-switch AA)
	  (sgp-fct (list :actiavtion-offsets 'FEAR-chunk-activation-offset 'SEEKING-chunk-activation-offset))
	  (schedule-periodic-event 0.016 'check-aur-loc-buffer-fear :module 'Affective-Associations :maintenance t)
	  (schedule-periodic-event 0.016 'check-vis-loc-buffer-fear :module 'Affective-Associations :maintenance t)
	  (schedule-periodic-event 0.100 'check-aur-buffer-fear :module 'Affective-Associations :maintenance t)
	  (schedule-periodic-event 0.100 'check-vis-buffer-fear :module 'Affective-Associations :maintenance t)
	  (schedule-periodic-event 0.016 'check-retr-buffer-fear :module 'Affective-Associations :maintenance t)))
	  )
...
(defun AA-clear-buffers (AA buffer chunk)
 (let* ((dm (get-module declarative))
  ...
  (when (not (AA-chunk-affect-switch AA))

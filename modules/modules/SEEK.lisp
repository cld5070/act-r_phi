...
(defun update-SEEKING (SEEKING)
	...
	(update-skinTemp)
	(let*
	 ((winnerVal (highest-SEEKING))
	 (sFChunk (if (chunk-p-fct (car winnerVal))
			   (mod-chunk-fct (car winnerVal) (list 'sFun (car winnerVal) 'sValue (cadr winnerVal)))
			   (define-chunks-fct (list (list (car winnerVal) 'isa 'sChunk 'sFun (car winnerVal) 'sValue (cadr winnerVal)))))))
...
(defun highest-SEEKING ()
 (let ((curr-val nil) 
      (curr-func nil)
	  (S (get-module SEEKING)))
  (maphash #'(lambda (k v) 
			  (when (or (not curr-val) (> (abs v) (abs curr-val)))
			   (setf curr-val v)
			   (setf curr-func k))) (SEEKING-S-vals S))
  (list curr-func curr-val)))

(defun update-hunger () 0)
...
(defun update-thirst ()
 ...
 (let* 
	...
	(checkOsmoVal-base (car(get-phys-vals t (list '("OsmBody.[Osm(mOsm/L)]-CellWall")))))

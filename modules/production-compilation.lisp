      ...
      (when (compilation-module-epl module)
        ...
        (bt:with-recursive-lock-held ((production-lock production))
          ...
          (cond ((not (valid-compilation-production p-name production))
                ...
                (t
                 (bt:with-recursive-lock-held ((production-lock (previous-production-struct (compilation-module-previous module))))
                  ...
                  (mapcan (lambda (x)
                            (let ((comp-type (get-compilation-type-struct x module)))
                              (when (and (comp-buffer-type-drop-out comp-type)
                                ...
                                (list (aif (chunk-copied-from-fct (cdr (assoc (read-from-string (format nil "=~a" x)) p2-bindings)))
                                           it
                                           (cdr (assoc (read-from-string (format nil "=~a" x)) p2-bindings)))))))
                    (production-lhs-buffers p2)))
                new-p)
  ...
  (bt:with-recursive-lock-held ((production-lock p))
    (dolist (old-p (all-productions) nil)
      (unless (equal old-p name)
        (let ((op (get-production old-p)))
          (bt:with-recursive-lock-held ((production-lock op))
            (when (equivalent-productions-p module op p)

    ...
    (dolist (event-check (meta-p-dynamics mp))
      (let ((event (car event-check))
        ...
        (when (and (< (act-r-event-mstime new-event) cur-time)
          ...
          (setf (meta-p-dynamics mp) (remove event-check (meta-p-dynamics mp)))
         ...
         (let ((mp (current-mp)))
           (bt:with-recursive-lock-held ((meta-p-schedule-lock mp))
             (let ((l (+ (length (meta-p-events mp)) (length (meta-p-dynamics mp)) (length (meta-p-delayed mp)))))
               (setf (meta-p-events mp) (remove event-num (meta-p-events mp) :key 'act-r-event-num :test '=))
               
               (setf (meta-p-dynamics mp) (remove event-num (meta-p-dynamics mp) :key (lambda (x) (act-r-event-num (car x))) :test '=))
               
               (setf (meta-p-delayed mp) (remove event-num (meta-p-delayed mp) :key 'act-r-event-num :test '=))

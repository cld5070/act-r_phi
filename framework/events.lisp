...
(defun external-evt-action (event)
  (aif (get-event-by-id event)
       (values (let ((action (act-r-event-action it)))
                 (if (stringp action) action (format nil "internal ~a" action)))
               t)
       (values nil nil)))
...
(defmethod format-event ((event fixnum))
  (aif (get-event-by-id event)
       (format-event it)
       nil))
...
(defconstant +format-event-break-event-string+ (formatter "~:[~*~*~;~6d.~3,'0d~] ~:[~*~;~a ~] ~:[~2*~;~va ~] ~va BREAK-EVENT ~@[~a~^ ~]"))

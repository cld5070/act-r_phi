...
(defun external-extend-chunks (parameter-name &optional params)
  (multiple-value-bind (valid ol) 
      (process-options-list params 'extend-chunks '(:default-value :default-function :merge-function :copy-function :copy-from-chunk-function))
    (when valid 
      (eval `(extend-chunks ,(string->name parameter-name) ,@(convert-options-list-items ol '(:default-function :merge-function :copy-function :copy-from-chunk-function) '(:default-value)))))))


(add-act-r-command "extend-chunks" 'external-extend-chunks 
                   "Create a new parameter for chunks. Params: parameter-name { < 'default-value',  'default-function', 'merge-function', 'copy-function', 'copy-from-chunk-function' > }."
                   nil)

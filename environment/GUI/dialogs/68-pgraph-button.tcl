...
proc draw_p_graph {win model clear_all {fname ""}} {
  ...
  if {$clear_all == 1} {
    ...
    if {$fname == ""} {
      ...
      add_cmd $warning_monitor "pg_record_warnings" "Environment command for capturing warnings during Load Production Graph."
...
proc save_p_graph_history_data {model} {
  ...
  if {$fname != ""} {
    ...
    if {$any == "nil" || $any == ""} {
      ...
      add_cmd $warning_monitor "pg_record_warnings" "Environment command for capturing warnings during Save Production History."

...
proc draw_p_trace {win model {file ""} {reuse 0}} {
  ...
  if {$file == ""} {
    ...
    add_cmd $warning_monitor "p_record_warnings" "Environment command for capturing warnings during Load Production Grid."
...
proc save_p_history_data {model} {
  ...
  if {$fname != ""} {
    ...
    if {$any == "nil" || $any == ""} {
      ...
      add_cmd $warning_monitor "p_record_warnings" "Environment command for capturing warnings during Save Production History."

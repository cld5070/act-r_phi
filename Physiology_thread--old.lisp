...
(defvar *HumModDir* (directory-namestring (make-pathname :directory (pathname-directory *LOAD-TRUENAME*))))
	...
	(let ((phys (get-module physio))
		...
		(while (probe-file solverInputFile))
		 ...
		 (handler-case (setf checkError (s-xml:parse-xml-file solverOutputFile)) 
				((or file-error s-xml::xml-parser-error) () nil))
...
(defun start-HumMod ()
	...
	(when (not (probe-file (concatenate 'string *HumModDir* "ModelSolver.exe"))) (error 'modelSolver-error :text (concatenate 'string *HumModDir* "ModelSolver.exe")))
	(setf *HumModProc* (run-program (concatenate 'string *HumModDir* "ModelSolver.exe") 
...
(defun create-phys-vars () 
 (tagbody
	...
	(let ((currTime (get-universal-time)))
	 ...
	 (while (and (probe-file solverOutputFile) (not (handler-case (delete-file solverOutputFile)
			(error () nil)))))
 ;Create input file for hummod to give us a list of variable and digest the output file created by HumMod (w/ the variables). Loop back around if there is an error
 (tagbody getVars
	(handler-case (delete-file solverOutputFile)
			(error () nil))
...
(let ((parseStart (get-universal-time)))
  (tagbody parseVarList
	...
	(handler-case (setf physVarList (s-xml:parse-xml-file solverOutputFile)) 
		((or file-error s-xml::xml-parser-error) () 
			(go parseVarList)))
	 (setf (phys-module-physVarList phys) physVarList)
	 (init-list-to-hash physVarList)
				...
				(loop
					...
					do (if (not(equal ':|solverout| k))
							(loop
								for k2 in k
								do (progn
										(if (equal ':|valuesframe| k2) (return-from create-phys-vars))
...
(if (< (mp-time) 0.006)
 ...
 (let* ((physio (get-module physio))
	   ...
	   (arousal 0)
	   (crh-max (phys-module-crh-max physio))
	   (crh-high-perc (phys-module-crh-high-perc physio))
	   (epi-max (phys-module-epi-max physio))
	   (epi-high-perc (phys-module-epi-high-perc physio))
...
(defun calc-crh-arousal (crh crh-base crh-high-perc crh-max)
 (declare (ignore crh-max))
 (declare (ignore crh-high-perc))

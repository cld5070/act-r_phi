...
;;;
;;; clear
;;;      It will be passed 1 additional parameter.  That parameter is the title of
;;;      a window.  This action indicates that all added items should be removed from
;;;      the window.
;;;
;;; attention 
...
(defun remove-virtual-window-handler (cmd)
  (bt:with-lock-held ((virtual-window-control-lock *vw-control*))
    (if (find cmd (virtual-window-control-handlers *vw-control*) :test 'string-equal :key 'car)
        (progn
          (setf (virtual-window-control-handlers *vw-control*)
            (remove cmd (virtual-window-control-handlers *vw-control*) :test 'string-equal :key 'car))
          t)
      nil)))
      ...
      (dolist (x (virtual-window-control-handlers *vw-control*) (when display (append display others)))
        (if (and (stringp (car x)) (local-or-remote-function-p (car x)))
            (if (cdr x)
              ...
              (push (car x) others))
          (progn
            (setf (virtual-window-control-handlers *vw-control*) (remove x (virtual-window-control-handlers *vw-control*) :key 'car))
